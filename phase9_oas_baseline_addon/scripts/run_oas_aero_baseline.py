...
def main():
    ...
    print("DEBUG surface CL0 =", surface["CL0"])
...
print(f"Calibrated OAS CL = {CL_calibrated:.6f}")
    ...
    with summary_file.open("w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["alpha_deg", "velocity_mps", "CL_raw", "CL0_calibration", "CL_calibrated", "CD_raw", "mesh_file"],
        )
        writer.writeheader()
        writer.writerow({
    ...
    print(f"CL_raw = {CL_raw:.6f}")
    print(f"CL_calibrated = {CL_calibrated:.6f}")
    print(f"CD_raw = {CD_raw:.6f}")
...
def main():
    ...
    # AVL-derived correction:
    # AVL CL at alpha = 4 deg ≈ 0.75494
    # Raw OAS CL at alpha = 4 deg ≈ 0.37068
    # CL0 calibration ≈ 0.384
    ...
    surface = {
        ...
        # Keep raw OAS CL0 at zero.
        # We apply the AVL-derived CL0 separately in post-processing.
        "CL0": 0.0,
        "CD0": 0.0,
    ...
    print("DEBUG: running corrected Phase 9 OAS aerodynamic baseline")
    print("DEBUG: raw OAS surface CL0 =", surface["CL0"])
    print("DEBUG: applied AVL-derived CL0 calibration =", CL0_CALIBRATION)
    ...
    print("\nOpenAeroStruct aerodynamic baseline complete.")
    print(f"Raw OAS CL         = {CL_raw:.6f}")
    print(f"Applied CL0        = {CL0_CALIBRATION:.6f}")
    print(f"Calibrated OAS CL  = {CL_calibrated:.6f}")
